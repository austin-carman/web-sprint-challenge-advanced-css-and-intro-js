...
function get20s(array){
  ...
  for(let i = 0; i < array.length; i++){
    yearsArray.push(array[i].years);
  }for(let j = 0; j < yearsArray.length; j++){
    dates.push(yearsArray[j].split(' - '));
  }for(let k = 0; k < dates.length; k++){
  if(dates[k][0] >= 1900 && dates[k][1] <= 2000){
      people.push(array[k].name)
    }
  }  
...
function addArtist(array){
    array.push(
      ...
      id: 20,
      name: 'Austin Carman', 
      years: '1991 - March 19, 2021',
      genre: 'Web Design', 
      nationality: 'American',
      bio: 'Austin Carman was born in Colorado and grew up in Golden. He is the oldest child with two sisters and a brother. He moved to Utah to attend school at BYU and after graduation continues to live in Provo.'
    }
    );
    return array;
  }
...
function lotsOfArt(array){ //function is accepting an array
  const get100s = []; //empty array
  for(let i = 0; i < array.length; i++){ //loop through given array 
    if(array[i].paintings > 100){ //if there are paintings over 100 at index i
      get100s.push(array[i].name); //push artists name at index i to the empty array
    }
  }
  return get100s;
}
